...
from SemKITTICurb_randla_block_pretrain_baseline_Dataloader import (
...
def build_dataloaders(args):
    ...
    val_loader = DataLoader(  # 不再用于正式 val 评估
        val_dataset,
        batch_size=args.batch_size,
        shuffle=False,
        num_workers=args.num_workers,
...
def run_one_epoch(
    ...
    with context:
        for batch in pbar:
            ...
            points = points.to(device, non_blocking=True).float()   # (B, N, 3)
    ...
    with torch.no_grad():
        for scene_idx in pbar:
            ...
            for start in range(0, len(blocks), batch_size):
                ...
                points_np = np.stack([b["points"] for b in batch_blocks], axis=0)   # (B, N, 3)
                labels_np = np.stack([b["labels"] for b in batch_blocks], axis=0)   # (B, N)

                points = torch.from_numpy(points_np).to(device, non_blocking=True).float()
                labels = torch.from_numpy(labels_np).to(device, non_blocking=True).long()
...
def parse_args():
    parser = argparse.ArgumentParser("RandLA-Net SemanticKITTI-Curb Training")
